...
globals [ weeds-poison-chance berry-energy berries-grow-rate berries-poison-chance grass-poison-chance ]
...
to setup
  ...
  grow-berries ;
  set weeds-poison-chance 0.9
  set berry-energy 10;
  set berries-grow-rate 2
  set berries-poison-chance 0.1
  set grass-poison-chance 0.05
  create-rabbits number [
    set color white
    setxy random-xcor random-ycor
    set energy random 10  ;start with a random amt. of energy
    set poisoned? false   ;start with the rabbit not being poisoned
    set recovery-time 0   ;start with no recovery time
    set gender one-of ["male" "female"] ; assign a gender to each rabbit
  ]
  reset-ticks
end
...
to eat-grass  ;; rabbit procedure
  ...
  [ set pcolor black
    ...
    if random-float 1.0 < grass-poison-chance ;
    [ set poisoned? true
      set recovery-time 3 ] ; set recovery time after getting poisoned
  ]
end
...
to eat-weeds  ;; rabbit procedure
  ...
  [ set pcolor black
    ...
    if random-float 1.0 < weeds-poison-chance ; add a chance of getting poisoned when eating weeds
    [ set poisoned? true
      set recovery-time 3 ] ] ; set recovery time after getting poisoned
end
...
to reproduce     ;; rabbit procedure
  ...
  if energy > birth-threshold and not poisoned? and any? other rabbits in-radius 1 with [gender != [gender] of myself and energy > birth-threshold and not poisoned?]
   [ if random-float 1.0 < 0.5 ; add 50% chance
    [ set energy energy / 2
      hatch 1 [ fd 1 set gender one-of ["male" "female"] ] ] ] ; assign a gender to each new rabbit
end
...
to eat-berries  ;; rabbit procedure
  ...
  [ set pcolor black
    ...
    if random-float 1.0 < berries-poison-chance ; add a chance of getting poisoned when eating berries
    [ set poisoned? true
      set recovery-time 3 ] ] ; set recovery time after getting poisoned
end
